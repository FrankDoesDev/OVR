...
export default function Navbar({
  ...
  const isActive = (v: PageView) => {
    ...
    if (v.type === 'category') return currentView.type === 'category' && currentView.slug === v.slug
    if (v.type === 'search') return currentView.type === 'search'
    if (v.type === 'settings') return currentView.type === 'settings'
    if (v.type === 'archive') return currentView.type === 'archive' || currentView.type === 'archive-date'
    return false
  }
  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 h-12 bg-[var(--bg-primary)] border-b border-[var(--border-primary)] flex items-center px-4">
        <button
          onClick={() => { onRefreshCategories(); setOpen(true) }}
      ...
      >
        ...
        <nav className="p-3 space-y-0.5 flex-1 overflow-y-auto">
          ...
          {categories.length > 0 && <div className="my-2 border-t border-[var(--border-primary)]" />}
          ...
          {categories.map((cat) => (
            <NavItem
              ...
              onClick={() => navigateAndClose({ type: 'category', slug: cat.slug })}
            />
          ))}
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      {active && <span className="ml-auto w-1 h-1 rounded-full bg-[var(--gaming)]" />}
    </button>
  )
}
